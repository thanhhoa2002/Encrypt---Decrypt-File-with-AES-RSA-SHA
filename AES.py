...
def rotate_row_left(row, n=1):
    return row[n:] + row[:n]
##
## trans string to 4x4 blocks of data
...
def encrypt_file(plantext_file_name,ciphertext_file_name,byte_key):
        # ghi chuỗi từ file
    with open(plantext_file_name, 'r') as file_plaintext:
...
def decrypt_file(ciphertext_file_name,new_plantext_file_name,byte_key):
        # Đọc chuỗi từ file
    with open(ciphertext_file_name, 'rb') as file_encrypt:
        cipher_text = file_encrypt.read()
    decrypted_text = dec(byte_key, cipher_text)
    with open(new_plantext_file_name, 'w') as file_new_plantext:
        file_new_plantext.write(decrypted_text.decode()) 
